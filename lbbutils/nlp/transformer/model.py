...
def get_pad_mask(seq, pad_idx):
    return (seq != pad_idx).unsqeeze(-2)


def get_subsequent_mask(seq):
    sz_b, len_s = seq.size()
    subsquent_mask = (1 - torch.triu(torch.ones((1, len_s, len_s), device=seq.device), diagonal=1)).bool()
...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask, return_atts=False):
        enc_slf_attn_list = []
        ...
        for enc_layer in self.layer_stack:
            ...
            enc_slf_attn_list += [enc_slf_attn] if return_atts else []
        if return_atts:
...
class Decoder(nn.Module):
    def __init__(self, n_trg_vocab, d_word_vec, n_layers, n_head,
                 d_k, d_v, d_model, d_inner, pad_idx,
...
class Transformer(nn.Module):
    def __init__(self, n_src_vocab, n_trg_vocab, src_pad_idx, trg_pad_idx,
        ...
        self.d_model = d_model
        self.encoder = Encoder(n_src_vocab=n_src_vocab, n_postion=n_position, d_word_vec=d_word_vec, d_model=d_model,
                               d_inner=d_inner, n_layer=n_layers, n_head=n_head, d_k=d_k,
                               d_v=d_v, pad_idx=src_pad_idx, dropout=dropout, scale_emb=scale_emb)
        self.decoder = Decoder(n_trg_vocab=n_trg_vocab, n_position=n_position, d_word_vec=d_word_vec, d_model=d_model,
                               d_inner=d_inner, n_layers=n_layers, n_head=n_head, d_k=d_k,
                               d_v=d_v, pad_idx=trg_pad_idx, dropout=dropout, scale_emb=scale_emb)
        self.trg_word_prj = nn.Linear(d_model, n_trg_vocab, bias=False)
        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
        assert d_model == d_word_vec, "dim should same"
